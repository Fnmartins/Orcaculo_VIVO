...
//#endregion
//#region ../../node_modules/type-fest/source/characters.d.ts
/**
...
```
*/
type StaticPartOfArray<T extends UnknownArray, Result extends UnknownArray = []> = T extends unknown ? number extends T['length'] ? T extends readonly [infer U, ...infer V] ? StaticPartOfArray<V, [...Result, U]> : Result : T : never; // Should never happen
/**
...
```
*/
type VariablePartOfArray<T extends UnknownArray> = T extends unknown ? T extends readonly [...StaticPartOfArray<T>, ...infer U] ? U : [] : never; // Should never happen
/**
...
type Integer<T> = T extends unknown // To distributive type
 ? IsInteger<T> extends true ? T : never : never; // Never happens
/**
...
@category Utilities
*/
type IsEqual$2<A, B> = [A] extends [B] ? [B] extends [A] ? _IsEqual<A, B> : false : false;
...
type Merge<Destination, Source> = Destination extends unknown // For distributing `Destination`
 ...
 ? If<IsEqual$2<Destination, Source>, Destination, _Merge<Destination, Source>> : never // Should never happen
 : never; // Should never happen
type _Merge<Destination, Source> = Simplify<SimpleMerge<PickIndexSignature<Destination>, PickIndexSignature<Source>> & SimpleMerge<OmitIndexSignature<Destination>, OmitIndexSignature<Source>>>;
//#endregion
//#region ../../node_modules/type-fest/source/string-to-number.d.ts
/**
...
```
*/
type ReadonlyKeysOfUnion<Union> = Union extends unknown ? keyof { [Key in keyof Union as IsEqual$2<{ [K in Key]: Union[Key]; }, { readonly [K in Key]: Union[Key]; }> extends true ? Key : never]: never; } : never;
/**
...
//#endregion
//#region ../../node_modules/type-fest/source/string-to-array.d.ts
/**
...
//#endregion
//#region ../../node_modules/type-fest/source/string-length.d.ts
/**
...
@category Template literal
*/
type StringLength$1<S extends string> = StringToArray<S>['length'];
//#endregion
...
type GreaterThan<A extends number, B extends number> = A extends number // For distributing `A`
 ...
 ? number extends A | B ? boolean : [IsEqual$2<A, PositiveInfinity>, IsEqual$2<A, NegativeInfinity>, IsEqual$2<B, PositiveInfinity>, IsEqual$2<B, NegativeInfinity>] extends (infer R extends [boolean, boolean, boolean, boolean]) ? Or<And<IsEqual$2<R[0], true>, IsEqual$2<R[2], false>>, And<IsEqual$2<R[3], true>, IsEqual$2<R[1], false>>> extends true ? true : Or<And<IsEqual$2<R[1], true>, IsEqual$2<R[3], false>>, And<IsEqual$2<R[2], true>, IsEqual$2<R[0], false>>> extends true ? false : true extends R[number] ? false : [IsNegative<A>, IsNegative<B>] extends (infer R extends [boolean, boolean]) ? [true, false] extends R ? false : [false, true] extends R ? true : [false, false] extends R ? PositiveNumericStringGt<`${A}`, `${B}`> : PositiveNumericStringGt<`${Absolute<B>}`, `${Absolute<A>}`> : never : never : never // Should never happen
 : never; // Should never happen
//#endregion
...
@see {Except}
*/
type Filter<KeyType, ExcludeType> = IsEqual$2<KeyType, ExcludeType> extends true ? never : (KeyType extends ExcludeType ? never : KeyType);
type ExceptOptions = {
  /**
...
@category Object
*/
type Writable$1<BaseType, Keys extends keyof BaseType = keyof BaseType> = BaseType extends ReadonlyMap<infer KeyType, infer ValueType> ? Map<KeyType, ValueType> : BaseType extends ReadonlySet<infer ItemType> ? Set<ItemType> : BaseType extends readonly unknown[] ?
// Handle array
...
@category Array
*/
type ArrayTail$1<TArray extends UnknownArray> = IfNotAnyOrNever<TArray, {
...
*/
type InternalIsUnion<T, U = T> = (IsNever<T> extends true ? false : T extends any ? IsEqual$2<U, T> extends true ? false : true : never) extends (infer Result) ?
...
) ? IfNotAnyOrNever<_Base, {
  ifNot: _ConditionalKeys<_Base, Condition>;
  ifAny: keyof _Base;
}> : never;
type _ConditionalKeys<Base, Condition> = keyof { [Key in (keyof Base & {}) // `& {}` prevents homomorphism
 as ExtendsStrict<Base[Key], Condition> extends true ? Key : never]: never; };
//#endregion
...
```
*/
type LessThanOrEqual<A extends number, B extends number> = GreaterThan<A, B> extends (infer Result) ? Result extends true ? false : true : never; // Should never happen
//#endregion
...
type ArraySlice<Array_ extends readonly unknown[], Start extends number = never, End extends number = never> = Array_ extends unknown // To distributive type
 ...
 : never // Never happens
 : never; // Never happens
type _ArraySlice<Array_ extends readonly unknown[], Start extends number = 0, End extends number = Array_['length']> = And<IsEqual$2<Start, never>, IsEqual$2<End, never>> extends true ? Array_ : number extends Array_['length'] ? VariableLengthArraySliceHelper<Array_, Start, End> : ArraySliceHelper<Array_, IsEqual$2<Start, never> extends true ? 0 : Start, IsEqual$2<End, never> extends true ? Array_['length'] : End>;
type VariableLengthArraySliceHelper<Array_ extends readonly unknown[], Start extends number, End extends number> = And<Not<IsNegative<Start>>, IsEqual$2<End, never>> extends true ? ArraySplice<Array_, 0, Start> : And<And<Not<IsNegative<Start>>, Not<IsNegative<End>>>, IsEqual$2<GreaterThan<End, Start>, true>> extends true ? ArraySliceByPositiveIndex<Array_, Start, End> : [];
...
//#endregion
//#region ../../node_modules/type-fest/source/split-on-rest-element.d.ts
/**
...
@category Array
*/
type SplitOnRestElement<Array_ extends UnknownArray, Options extends SplitOnRestElementOptions = {}> = Array_ extends unknown // For distributing `Array_`
 ? IfNotAnyOrNever<Array_, {
  ifNot: _SplitOnRestElement<Array_, ApplyDefaultOptions<SplitOnRestElementOptions, DefaultSplitOnRestElementOptions, Options>>;
}> extends (infer Result extends UnknownArray) ? If<IsArrayReadonly<Array_>, Readonly<Result>, Result> : never // Should never happen
 : never; // Should never happen
/**
Deconstructs an array on its rest element and returns the split portions.
*/
type _SplitOnRestElement<Array_ extends UnknownArray, Options extends Required<SplitOnRestElementOptions>, HeadAcc extends UnknownArray = [], TailAcc extends UnknownArray = []> = keyof Array_ & `${number}` extends never ?
...
Array_ extends readonly [...infer Rest, infer Last] ? _SplitOnRestElement<Rest, Options, HeadAcc, [Last, ...TailAcc]> // Accumulate elements that are present after the rest element.
 : [HeadAcc, Array_ extends readonly [] ? [] : Array_, TailAcc] // Add the rest element between the accumulated elements.
 : Array_ extends readonly [(infer First)?, ...infer Rest] ? _SplitOnRestElement<Rest, Options, [...HeadAcc, ...IsOptionalKeyOf<Array_, '0'> extends true ? Options['preserveOptionalModifier'] extends false ? [If<IsExactOptionalPropertyTypesEnabled, First, First | undefined>] // Add `| undefined` for optional elements, if `exactOptionalPropertyTypes` is disabled.
 : [First?] : [First]], TailAcc> // Accumulate elements that are present before the rest element.
...
@category Template literal
*/
type LastArrayElement<TArray extends UnknownArray> = IfNotAnyOrNever<TArray, {
  ifNot: TArray extends UnknownArray // For distributing `TArray`
   ? SplitOnRestElement<TArray> extends readonly [infer BeforeRest extends UnknownArray, infer Rest extends UnknownArray, infer AfterRest extends UnknownArray] ? _LastArrayElement<BeforeRest, Rest, AfterRest> : never : never;
}>;
type _LastArrayElement<BeforeRest extends UnknownArray, Rest extends UnknownArray, AfterRest extends UnknownArray> = AfterRest extends readonly [...any, infer Last] // Note there are no optional elements in `AfterRest`.
...
type BeforeRestLastElement<BeforeRest extends UnknownArray, Accumulator = never> = BeforeRest extends readonly [] ? Accumulator | undefined : BeforeRest extends readonly [...any, infer Last] ? Last | Accumulator : BeforeRest extends readonly [...infer Rest, (infer Last)?] ? BeforeRestLastElement<Rest,
// Add `undefined` for optional elements, if `exactOptionalPropertyTypes` is disabled.
Last | Accumulator | If<IsExactOptionalPropertyTypesEnabled, never, undefined>> : never;
//#endregion
...
type FallbackOf<T> = IsEqual$2<T, NonNullable<T>> extends true ? RemedaTypeError<"defaultTo", "no unnecessary fallback", {
  type: never;
  metadata: T;
}> : T;
/**
...
type IsEqual$1<T, Other> = (data: T, other: Other) => boolean;
/**
 ...
 * @category Array
 */
declare function differenceWith<T, Other>(data: readonly T[], other: readonly Other[], isEqual: IsEqual$1<T, Other>): T[];
/**
 * Excludes the values from `other` array.
 * Elements are compared by custom comparator isEquals.
 *
 ...
 * @category Array
 */
declare function differenceWith<T, Other>(other: readonly Other[], isEqual: IsEqual$1<T, Other>): (data: readonly T[]) => T[];
//#endregion
 ...
 */
type FilteredFixedTuple<T, Condition> = T extends readonly [infer Head, ...infer Rest] ? Head extends Condition ? [Head, ...FilteredFixedTuple<Rest, Condition>] : FilteredFixedTuple<Rest, Condition> | (IsNever<SymmetricRefine<Head, Condition>> extends true ? never : [SymmetricRefine<Head, Condition>, ...FilteredFixedTuple<Rest, Condition>]) : [];
/**
 ...
 */
type RefineIncomparable<Item, Condition> = IsRealObject<Item> extends true ? IsRealObject<Condition> extends true ? IsNever<Extract<keyof Item, keyof Condition>> extends true ? never : Item & Condition : never : never;
type IsRealObject<T> = T extends object ? T extends readonly unknown[] ? false : true : false;
//#endregion
 ...
 * @category Array
 */
declare function filter<T extends IterableContainer, Condition>(data: T, predicate: (value: T[number], index: number, data: T) => value is Condition): FilteredArray<T, Condition>;
 ...
 * @category Array
 */
declare function filter<T extends IterableContainer, Condition>(predicate: (value: T[number], index: number, data: T) => value is Condition): (data: T) => FilteredArray<T, Condition>;
...
type WithSameReadonly<Source, Destination> = IsEqual$2<Source, Readonly<Source>> extends true ? Readonly<Destination> : Destination;
...
type HasSubObjectGuard<T, S> = Simplify<Tagged<S & T, typeof BRAND_HAS_SUB_OBJECT>>;
 ...
 * @category Array
 */
declare function intersectionWith<TFirst, TSecond>(array: readonly TFirst[], other: readonly TSecond[], isEqual: IsEqual<TFirst, TSecond>): TFirst[];
/**
 * Returns a list of intersecting values based on a custom
 * comparator function that compares elements of both arrays.
 *
 ...
 */
isEqual: IsEqual<TFirst, TSecond>): (array: readonly TFirst[]) => TFirst[];
//#endregion
...
type HasWritableKeys<T> = IsEqual$2<Readonly<T>, T> extends true ? false : true;
//#endregion
...
type ShouldNotNarrow<T> = unknown extends T ? true : IsAny<T> extends true ? true : IsEqual$2<T, {}> extends true ? true : false;
/**
 ...
 * @category Guard
 */
declare function isNot<T extends (data: unknown) => data is unknown>(predicate: IsUnknown<GuardType<T>> extends true ? (data: unknown) => data is never : T): <Wide>(data: Wide) => data is Exclude<Wide, GuardType<T>>;
declare function isNot<T, Narrow extends T>(predicate: (data: T) => data is Narrow): (data: T) => data is Exclude<T, Narrow>;
 ...
 * @category Array
 */
declare function last<T extends IterableContainer>(data: T): LastArrayElement<T>;
/**
 * Gets the last element of `array`.
 *
 * @signature
...
type FixedTupleSplits<L, R extends unknown[] = []> = (L extends readonly [...infer Head, infer Tail] ? FixedTupleSplits<Head, [Tail, ...R]> : never) | {
  left: L;
  right: R;
};
//#endregion
...
type Resolved<T> = T extends (infer U) ? U : never;
//#endregion
...
type Prop<T, Key> = T extends unknown ? Key extends keyof T ? T extends readonly unknown[] ? ArrayAt<T, Key> : T[Key] | (Key extends keyof OmitIndexSignature<T> ? never : undefined) : undefined : never;
 ...
 * @category Object
 */
declare function prop<T extends NonPropertyKey, Key extends KeysDeep<T, []>>(data: T, key: Key): Resolved<Prop<T, Key>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>>(data: T, key0: Key0, key1: Key1): Resolved<PropDeep<T, [Key0, Key1]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>>(data: T, key0: Key0, key1: Key1, key2: Key2): Resolved<PropDeep<T, [Key0, Key1, Key2]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>>(data: T, key0: Key0, key1: Key1, key2: Key2, key3: Key3): Resolved<PropDeep<T, [Key0, Key1, Key2, Key3]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>>(data: T, key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4): Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>>(data: T, key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5): Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>>(data: T, key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6): Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>, Key7 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>>(data: T, key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6, key7: Key7): Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>, Key7 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>, Key8 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7]>>(data: T, key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6, key7: Key7, key8: Key8): Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7, Key8]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>, Key7 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>, Key8 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7]>, Key9 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7, Key8]>, AdditionalKeys extends readonly PropertyKey[] = []>(data: T, key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6, key7: Key7, key8: Key8, key9: Key9, ...additionalKeys: AdditionalKeys): Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7, Key8, Key9, ...AdditionalKeys]>>;
/**
 * Gets the value of the given property from an object. Nested properties can
 * be accessed by providing a variadic array of keys that define the path from
 * the root to the desired property. Arrays can be accessed by using numeric
 ...
 * @category Object
 */
declare function prop<T extends NonPropertyKey, Key extends KeysOfUnion<T>>(key: Key): (data: T) => Resolved<Prop<T, Key>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>>(key0: Key0, key1: Key1): (data: T) => Resolved<PropDeep<T, [Key0, Key1]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>>(key0: Key0, key1: Key1, key2: Key2): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>>(key0: Key0, key1: Key1, key2: Key2, key3: Key3): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2, Key3]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>>(key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>>(key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>>(key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>, Key7 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>>(key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6, key7: Key7): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>, Key7 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>, Key8 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7]>>(key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6, key7: Key7, key8: Key8): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7, Key8]>>;
declare function prop<T extends NonPropertyKey, Key0 extends KeysDeep<T, []>, Key1 extends KeysDeep<T, [Key0]>, Key2 extends KeysDeep<T, [Key0, Key1]>, Key3 extends KeysDeep<T, [Key0, Key1, Key2]>, Key4 extends KeysDeep<T, [Key0, Key1, Key2, Key3]>, Key5 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4]>, Key6 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5]>, Key7 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6]>, Key8 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7]>, Key9 extends KeysDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7, Key8]>, AdditionalKeys extends readonly PropertyKey[] = []>(key0: Key0, key1: Key1, key2: Key2, key3: Key3, key4: Key4, key5: Key5, key6: Key6, key7: Key7, key8: Key8, key9: Key9, ...additionalKeys: AdditionalKeys): (data: T) => Resolved<PropDeep<T, [Key0, Key1, Key2, Key3, Key4, Key5, Key6, Key7, Key8, Key9, ...AdditionalKeys]>>;
 ...
 * @category Utility
 */
declare function stringToPath<const S extends string>(stringPath: S): StringToPath<S>;
//#endregion
...
type isLessThan<A extends number, B extends number> = IsEqual$2<A, B> extends true ? false : 0 extends A ? true : 0 extends B ? false : isLessThan<Difference<A, 1>, Difference<B, 1>>;
...
type TitleCase<S extends string, Options extends TitleCaseOptions> = IsLiteral<S> extends true ? Join<TitleCasedArray<Words<IsEqual$2<S, Uppercase<S>> extends true ? Lowercase<S> : S>, TitleCaseOptionsWithDefaults<Options>>, " "> : string;
